...
- (void)rollTo:(NSPoint)pt sender:(NSView*)sender
{
    ...
    sinAng = lr = sqrt(rot[1]*rot[1] + rot[2]*rot[2] + rot[3]*rot[3]); // ||(s X e)||;
                                // keep this length in lr for norm3Dalizing the rotation vector later.
    sinAng = sinAng * ls * le;
    rot[0] = (float)atan2(sinAng, cosAng) * kRad2Deg; // GL rotations are in degrees.
    
    // norm3Dalize the rotation axis.
    lr = 1. / lr;
...
- (void)add:(float *)dA toRotation:(float *)A
{
    ...
    // Here's an excersize for the reader: it's a good idea to re-norm3Dalize your quaternions
